...
run_cse_verify() {
  ...
  lab_log "CSE verify OK site=${SITE_ID}"
}
...
run_proofs() {
  ...
  /tmp/internet-proof-on-host.sh "$SITE_ID" || return 1
  s3_fetch_script routing-proof-on-host.sh /tmp/routing-proof-on-host.sh
  /tmp/routing-proof-on-host.sh all || return 1
}
      ...
      start)
        if [[ "$SITE_ID" == "0" ]]; then
          ...
          for _ in $(seq 1 60); do
            [[ -f "${LAB_STATE_DIR}/sites.env" ]] && imds_tag PeerTransportEniIp | grep -q . && break
            sleep 30
          done
        fi
        set_phase cse
