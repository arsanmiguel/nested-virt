...
# CSE finding: "Recursive DNS Server Exposed to Internet" — system dnsmasq.service on :53.
...
disable_system_dnsmasq() {
  ...
  for pid in $(pgrep -x dnsmasq 2>/dev/null || true); do
    _is_lab_dnsmasq_pid "$pid" && continue
...
# Exit 0 if no DNS listener on non-loopback addresses (CSE-safe).
...
# Do not enable recursion or bind to kvm-host-nic0 — CSE: recursive DNS exposed.
port=0
bind-interfaces
...
# Lab dnsmasq is DHCP-only (port=0 for CSE). Do not point DNS at gateway — nothing listens on :53.
