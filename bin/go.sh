#!/usr/bin/env bash
# Nested-virt — single entry point for deploy, hardening, guests, L2, and proofs.
#
...
for s in json.load(sys.stdin).get('StackSummaries', []):
    ...
    if n.startswith(base + '-') and st != 'DELETE_COMPLETE':
        print(n)
" "$base")
    while IFS= read -r s; do [[ -n "$s" ]] && delete_stack "$s"; done <<< "$stacks"
  done
...
# ── CSE hardening (dnsmasq port=0, VNC localhost) ───────────────────────────
...
verify_cse_site() {
  ...
  [[ "$fail" -eq 0 ]] || { echo "Site ${sid} CSE FAILED" >&2; return 1; }
  echo "Site ${sid} CSE OK"
}
