...
Scripts land on metal via **S3 bootstrap bucket** + SSM. **Win2022.iso** and **virtio-win.iso** prefetch over **HTTPS** during bootstrap (Microsoft CDN + fedorapeople by default). Windows is configured over **WinRM**; inner VM disk/seed served over **HTTP from lab gateway**.
