# Nested-virt developer guide

Guide for the two-AZ nested virtualization lab on AWS bare metal - architecture, build sequence, scripts, and verification.

**Related:** [README](../README.md) · [Operator deploy](DEPLOY-FROM-CFN.md) · [Troubleshooting](nested-virt-hiccups.md) · [Network topology](network-diagram.md) · [Security design](SECURITY-EXCEPTIONS.md)
...
| Path | Who | How |
|------|-----|-----|
| **Drop-in CFN** | Operators | Upload [`nested-virt-lab.yaml`](../cloudformation/nested-virt-lab.yaml) - one stack, both sites. See [DEPLOY-FROM-CFN.md](DEPLOY-FROM-CFN.md). |
| **Developer workflow** | Contributors | Clone repo, `config.env`, per-site stacks via `bin/run-site.sh` or `./bin/go.sh`. Scripts upload to S3; same on-metal pipeline. |

Both paths run the same bootstrap and on-instance pipeline. The drop-in template bundles scripts; the developer path lets you iterate locally before regenerating the template.
...
3. **Operator deploy:** [DEPLOY-FROM-CFN.md](DEPLOY-FROM-CFN.md) - upload `nested-virt-lab.yaml`, confirm GREEN via SSM or `./bin/check-lab-status.sh`.
4. **Full developer deploy:** `./bin/go.sh --fresh` - teardown, per-site CFN stacks, bootstrap, guests, L2, lab security verify, all proofs (~3-4 hours including L2 VHDX per site).
5. **Resume / idempotent:** `./bin/go.sh` - skips completed phases, refreshes inner VHDX if internet proof fails.
6. **Proofs only:** `./bin/invoke-routing-proof.sh --layer all` (includes `--layer internet`).
7. **Poll until GREEN (operators):** `./bin/monitor-lab-until-green.sh` after CFN deploy.
...
**Verification:** Drop-in CFN per [DEPLOY-FROM-CFN.md](DEPLOY-FROM-CFN.md). Confirm GREEN via `./bin/check-lab-status.sh` (rejects stale SSM) or SSM `/nested-virt/lab/verification`.
...
| **Win2022 ISO over HTTPS** | External deployers download from Microsoft CDN at bootstrap; no cross-account IAM |

Patterns borrowed from bare-metal launch work (bridge naming, `PHASE=` timing logs in `/var/log/amazon/launch-timing.log`, extra ENIs per AZ).
...
2. **`bootstrap.sh`** on each host:
 ...
 - Picks data NVMe by size, not device order (hiccup #6)
...
5. **`provision-windows-guest.sh`**
 ...
 - Uses `Win2022.iso` from `/var/lib/libvirt/images` (prefetched by `ensure-lab-image-cache.sh` over HTTPS or S3)
 - Builds autounattend floppy with site-specific IP
 - **dnsmasq** on `br-default` with reservations for Windows (`.10`) and inner (`.20`) MACs - **DHCP only** (`port=0`); system `dnsmasq.service` masked (`scripts/ensure-lab-dnsmasq.sh`) - hiccups #7, #8, #20
 - **`virt-install`** → `win-hv-nested` on `br-default`, **e1000**, MAC `52:54:00:10:00:{site}0`
 - CPU model **`Cascadelake-Server-noTSX`** (critical on 8488C - see hiccup #17)

6. **Autounattend** defers Hyper-V hypervisor install to day-2 script (hiccup #16)
...
Detailed entries live in [nested-virt-hiccups.md](nested-virt-hiccups.md) (see **Quick index by phase**). Highlights:
...
Skylake CPU model in KVM XML caused Automatic Repair loop. Fix: **`Cascadelake-Server-noTSX`**. See hiccup #17.
...
Fix: HD-only boot + **`virt-customize` netplan** + Ubuntu **24.04** + `-ForceReinstall` deletes disk/seed. See hiccup #18.
...
| `scripts/security-scan.sh` | Laptop | CSE handoff: Trivy + Checkov on `template-src.yaml` and `nested-virt-lab.yaml`, Gitleaks, optional cfn-lint/shellcheck (Docker/Colima) |
...
```bash
# Operators (drop-in CFN stack - clone optional for helper scripts)
./bin/check-lab-status.sh              # exit 0 = SSM GREEN matching live stack; 2 = stale/running
./bin/monitor-lab-until-green.sh       # poll until GREEN or RED (uses check-lab-status)

# Developers (needs sites.env + AWS creds)
...
| Doc | Contents |
|-----|----------|
| [DEPLOY-FROM-CFN.md](DEPLOY-FROM-CFN.md) | Operator drop-in CFN deploy |
| [network-diagram.md](network-diagram.md) | Color-coded topology, packet walk, proof layers |
| [nested-virt-hiccups.md](nested-virt-hiccups.md) | 22 tripping points with fixes (index by pipeline phase) |
...
| [SECURITY-EXCEPTIONS.md](SECURITY-EXCEPTIONS.md) | Intentional lab security choices; **CSE scan handoff** |
...
Port this lab to Terraform - see [ROADMAP.md](ROADMAP.md). CloudFormation + shell scripts remain the reference implementation until then.

---

*Built for nested virt on AWS metal - proven layer by layer.*
